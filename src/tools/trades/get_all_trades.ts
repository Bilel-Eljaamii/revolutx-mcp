...
export const getAllTradesTool: Tool = {
  name: "get_all_trades",
  description:
    "Retrieve a list of all public trades (market history) for a specific symbol, not limited to the current client's activity.",
  inputSchema: {
    type: "object",
    properties: {
      symbol: {
...
export async function handleGetAllTrades(args: unknown) {
  ...
  const path = `/api/1.0/trades/${symbol}`;
  const url = queryString
    ? `${REVOLUTX_API_URL}/trades/${symbol}?${queryString}`
    : `${REVOLUTX_API_URL}/trades/${symbol}`;
  ...
  try {
    ...
    return handleAxiosError(error, `fetching all trades for ${symbol}`);
  }
}

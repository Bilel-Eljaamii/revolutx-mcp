...
export const prompts: Prompt[] = [
  ...
  },
  {
    ...
    description:
      "Generate a ladder trading strategy (multiple orders at different price levels).",
    arguments: [
      {
        name: "symbol",
    ...
    arguments: [
      {
        name: "symbols",
        description:
          "Comma-separated list of trading pairs (e.g., BTC-USD,ETH-USD,SOL-USD)",
        required: true,
      },
    ],
  },
  {
    ...
    arguments: [
      ...
      },
      {
        name: "direction",
        description:
          "Direction to monitor: 'above' or 'below' (default: above)",
        required: false,
      },
    ],
  },
...
export function handleGetPrompt(name: string, args: unknown) {
  ...
  if (name === "analyze-market") {
    const symbol = promptArgs.symbol;
    if (!symbol) {
...
3. Summarize the current sentiment (bullish/bearish/neutral) and key price levels.`,
  ...
  if (name === "create-ladder-strategy") {
    const { symbol, start_price, end_price, num_levels, total_quantity, side } =
      promptArgs;
    if (
      !symbol ||
      !start_price ||
      ...
      throw new Error("Missing required arguments for create-ladder-strategy");
    }
    return {
      messages: [
        {
...
3. Ask for confirmation before using 'place_order' for each level.`,
  ...
  if (name === "portfolio-summary") {
    const currency = promptArgs.currency ?? "USD";
    return {
      messages: [
        {
          role: "user",
...
4. Ask if I want to cancel any specific orders (use 'cancel_order' tool if confirmed).`,
  ...
  if (name === "market-comparison") {
    const symbols = promptArgs.symbols;
    if (!symbols) {
...
Recommend which pairs offer the best trading conditions.`,
  ...
  if (name === "price-alert-setup") {
    const { symbol, target_price, direction } = promptArgs;
    if (!symbol) {
      throw new Error("Argument 'symbol' is required");
    }
    if (!target_price) {
